...
describe('Post', () => {
    ...
    const testPost = {
        ...
        "charityName": "HolyHands",
        "name": "Trevor",
        "amountGiven": "$967.32",
        "description": "This charity has really helped me cope."
    ...
    it('should update a single post on /posts/<id> PUT', (done) => {
        ...
        post.save((err, post) => {
            chai.request(server)
            .put(`/posts/${post._id}`)
            .send({ 'charityName': 'PracticalPaws'})
                .end((err, res) => {
                    res.should.have.status(200)
                    res.should.be.html
                    done();
    ...
    after(() => {
        Post.deleteMany({ charityName: 'HolyHands', charityName: 'PracticalPaws' })
            .exec((err, post) => {
                console.log(post)
                post.remove();
        });
